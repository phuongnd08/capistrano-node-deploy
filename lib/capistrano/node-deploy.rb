...
script
    cd #{current_path} && PID=#{shared_path}/pids/#{pid_file} NODE_ENV=#{node_env} #{app_environment} #{node_binary} #{current_path}/#{app_command} 2>> #{shared_path}/#{node_env}.err.log 1>> #{shared_path}/#{node_env}.out.log
end script
EOD
  }
